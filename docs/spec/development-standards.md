...
> AdaWorks /AdaWorks AI前后端统一约定。新代码与重构均须遵守。
...
| Backend API     | `backend/adaworks/api/`      | 路由与契约           |
| Backend Service | `backend/adaworks/services/` | 业务逻辑、LLM、日志  |
...
推荐工具：`backend/adaworks/http_safe.register_global_exception_handler(app)`。
