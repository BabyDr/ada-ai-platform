...
> 策略：**现有 Linguist AI / AdaAgent 增量改造**，UI 不动，底层接入 SSE task 契约
...
- **后端在 adaagent 包内扩展**：`backend/adaagent/` 增量新增模块，保留会话/聊天/WebSocket/日志等已有路由。
...
> 目标：在 `backend/adaagent/` 内新增 task 契约，mock 模式可流式输出。依赖现有 FastAPI 入口。

### T1.1 [P] 配置管理 `adaagent/config.py` [F9]
- 交付物：`backend/adaagent/config.py`（`Settings` + `settings` 单例）。
...
- 交付物：`backend/adaagent/services/llm.py`（`LLMService.stream()` + `_mock_stream()`）。
...
### T1.4 [P] Prompt 模板 [F9]
- 交付物：`backend/adaagent/services/prompt.py`（承接/迁移 `linguist_service` 翻译与总结 prompt）。
...
### T1.5 请求/响应模型
- 交付物：`backend/adaagent/api/schemas.py`（`TaskCreateRequest`、`FunctionItem`、`FunctionsResponse` 等）。
...
- 交付物：`backend/adaagent/api/functions.py` + 注册到 `main.py`。
...
- 交付物：`backend/adaagent/api/task.py`（`create_task` + `_task_generator`；事件 `task_start` / `token` / `task_done` / `task_error`）。
...
- 交付物：`backend/adaagent/services/task_manager.py`（状态机 pending→running→done/failed/cancelled）。
