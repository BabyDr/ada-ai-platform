...
> **核心约束**：在现有 Linguist AI 工作区代码上改造，**不重做 UI**；导航从 `activeView` 组件切换**迁移为 Vue Router**。
...
| **后端在 adaagent 包内扩展** | 在 `backend/adaagent/` 现有 `main.py` 基础上新增 `services/`、`api/` 等模块，保留会话/聊天/日志等已有路由 |
  ...
  "functions": [
    {
      ...
      "params": { "text": "string", "sourceLang": "string", "targetLang": "string", "tone": "string" }
    },
    {
      "id": "summarize",
      "name": "智能要点总结",
      "description": "长文本总结，支持要点数/字数上限/语调",
      "params": { "text": "string", "keyPointsCount": "number", "wordLimit": "number", "tone": "string" }
    }
  ]
}
```
...
- [ ] `adaagent/config.py` + `services/llm.py`（mock 模式）+ `services/prompt.py`
...
│       └── types.ts                 # 已有
│
├── backend/                         # Python FastAPI Sidecar（现有 adaagent 包 + 增量模块）
│   └── adaagent/
